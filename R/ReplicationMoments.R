...
n<-dim(Z)[1];
k<-length(betap);
betap<-as.matrix(betap,length(betap),1); # @TSA: betap in matrix umgewandelt /Transpose?. Auch in anderen Skripten nötig?
...
moments <- lmoments/matrix(rep(phat,dim(lmoments)[2]),ncol=dim(lmoments)[2],byrow=FALSE)
