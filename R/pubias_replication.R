...
pubias_replication <- function(data, studynames, symmetric = 1, cutoffs = 1.96, GMM = FALSE, print_plots = FALSE){
  ...
  if (GMM == TRUE) {
    ...
    rmarkdown::render("R/dashboard.Rmd", params = list(plots=plots, descriptives = descriptives, pub_prob = result$Psihat),output_file = paste0(getwd(), "/", name,"_Dashboard.html"))
    ...
    descriptives <- descriptive_stats(Z, sigmaZ2, identificationapproach, name, symmetric, cluster_ID)
    plots <- plot_correction(Z,sigmaZ2,Psihat,Varhat,cutoffs,symmetric,symmetric_p,studynames,identificationapproach, corrected_estimates)
    rmarkdown::render("R/dashboard.Rmd", params = list(plots=plots, descriptives = descriptives, pub_prob = result$Psihat[-c(1,2)]),output_file = paste0(getwd(), "/", name,"_Dashboard.html"))
