...
If the functions are run with the option to print the plots and dashboard, the result list also contains the `ggplot` objects for the descriptive as well as the correction plots. The plots will additionally be printed as .pdf into the current working directory. If specified, a HTML-Dashboard with dynamic plots will be exported as well. One can also look at the plots by calling: 
...
Looking at the estimation results indicates a publication probability of around 25% suggesting, that statistically significant results are 4 times more likely to be included in the meta-study by Croke et al. (2016). But because we see no clear evidence for publication probability in the descriptives and deal with a high robust standard error for the publication probability, we can not draw any final conclusions.
...
Again, in Figure 9, we can see the biggest difference between the unbiased estimates and the original estimates around the cutoff 1.96. Above and estimation of 4, the adjusted equal the original estimates. The same holds for the estimates close to 0.
